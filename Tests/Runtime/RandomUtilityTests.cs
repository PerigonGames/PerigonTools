...
namespace Tests
{
    ...
    {
        private RandomUtility _random = null;
        [SetUp]
        public void Setup()
        {
            _random = new RandomUtility();
        }
        ...
        [Test]
        public void TryGetRandomElementWithArrayOfOneInt()
        {
            //Arrange
            var array = new[] { 1 };
            ...
            if (_random.NextTryGetElement(array, out var actualResult))
            {
                Assert.AreEqual(actualResult, 1, "Value taken should be 1");
            }
